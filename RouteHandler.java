...
public class RouteHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) {
      ...
      else if(((this.contentType.indexOf("image") != -1) || (this.contentType.indexOf("pdf") != -1)) && (this.file != null)){
        exchange.getResponseHeaders().set("Content-Type", this.contentType);
        this.responseSpecial = Input.readFileSpecial(this.file);
        num ++;
      }
      try{
        if(num == 0){
          ...
          send(this.responseSpecial,exchange,this.route);
        }
      }catch(IOException e){
    ...
    try{
      ...
      lg.dispMessage(writer.toString(), "warn");
    ...
    try{
      ...
      Writer writer = new StringWriter();
      e.printStackTrace(new PrintWriter(writer));
      lg.dispMessage("Send: " + writer.toString(), "error");
    }
	}

  public static void send(byte[] responseSpecial, HttpExchange exchange, String routeStr) throws IOException{
    if(!exchange.getResponseHeaders().containsKey("Access-Control-Allow-Origin")){
      exchange.getResponseHeaders().add("Access-Control-Allow-Origin","*");
    }
    String remoteAddress;
    ...
    try{
        exchange.sendResponseHeaders(200, responseSpecial.length);
        OutputStream os = exchange.getResponseBody();
    
        os.write(responseSpecial);
        os.close();
    }catch(IOException e){
      Writer writer = new StringWriter();
      e.printStackTrace(new PrintWriter(writer));
